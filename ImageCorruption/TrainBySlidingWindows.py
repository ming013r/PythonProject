...
def retrieveFromTxt():
    trainList = pd.read_csv('txtSrc/trainData.csv').sample(2000)#這裡可以調

    dirtylist = numpy.array(trainList)[:,0:9]
    clean_targetlist = numpy.array(trainList)[:,9]
...
def train(data):
    ...
    lmnet = algorithms.LevenbergMarquardt((9, 20, 1))#這裡可以調
    lmnet.train(dirtylist, clean_targetlist,epochs=100,epsilon=0.01) #這裡可以調
